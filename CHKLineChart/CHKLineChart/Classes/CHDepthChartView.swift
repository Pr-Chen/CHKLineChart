...
extension CHDepthChartView {
    ...
    fileprivate func drawYAxisLabel(_ yAxisToDraw: [(CGRect, String)]) {
        ...
        switch self.showYAxisLabel {
        case .left:
            alignmentMode = self.isInnerYAxis ? .left : .right
        case .right:
            alignmentMode = self.isInnerYAxis ? .right : .left
        case .none:
            alignmentMode = .left
        }
